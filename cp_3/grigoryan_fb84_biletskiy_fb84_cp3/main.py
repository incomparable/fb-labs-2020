...
def analiz(text):
    ...
    if ok > 2:
        ...
        return False
    # bigrcount = 0
    # bigramss = ['СТ', 'НО', 'ЕН', 'ТО', 'НА', 'ОВ', 'НИ', 'РА', 'ВО', 'КО',
...
def decoder(keyslist, name_of_text):
    text = open(name_of_text, "r", encoding="utf8")
    text = text.read()
    textlen = len(text)
    buff_text = ''
    # keyslist = [(123,33),(123,232),(2323,323),(27,211)]
    for a, b in keyslist:
...
f = openfile("text.txt")
bigr = bigrams(f)
perebor(popbigr, bigr)
decoder(keylist, "filtered.txt")
